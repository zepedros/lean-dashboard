...
In this report, we explain the implemented solution to the problem at hands, as well as provide a more in-depth analysis of the architecture and technology choices.
...
On this section it is mostly explained the decisions made when implementing the client application, aswell as what each page of the application is used for and how they function.
...
React has very powerful tools to use when writing code for an application. These tools are know as "Hooks".
Hooks are functions that allow us to “hook into” React state and lifecycle features from function components. Most of the hooks are built-in with the React library.
\newline
On our client application we use 4 main hooks, these being:
 \begin{itemize}
	\item useState:

This hook is used to store state within our components, we mostly used it to save the data we fetch from our API and the input for the application.
	\item useEffect:
...
This hook is use to store context within our application. Within our application we use it to save the session context of the application. Whenever a user logs on to the application, their credentials are stored as context and it is used to ensure the user is still logged onto the application.
	\item useFetch: useFetch is a custom hook that allows us to make fetch requests to our API.
\end{itemize}
...
The page component is where all of the smaller components are organised. 
\newline
...
We provide 4 types of different widgets:
 \begin{itemize}
	...
	\item Pie Chart
\end{itemize}
These widgets were developped in such a way that can adapt to any form of data that is sent to them, for example, the Data Table will have the necessary columns to display the data and the Pie Chart will have as many divisions as required.
...
After this has been decided, the user is presented with a dialogue with the required widget informations.
...
When the user comfirms this action they are redirected to the dashboard page and can see the newly added widget.
